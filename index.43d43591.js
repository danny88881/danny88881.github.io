...
const H = new (0, _highwayDefault.default).Core({
    ...
    renderers: {
        ...
        maingamegallery: (0, _gameGallerySetupDefault.default)
    }
});
...
class Particle {
    ...
    draw(ctx) {
        ...
        var hexString = (255 - Math.floor(Math.min(distFromBase / 1000, 1) * 255)).toString(16);
        if (hexString.length < 2) hexString = "0" + hexString;
        ctx.fillStyle = col; // + hexString;//interpolate(col, "#000000", Math.min(distFromBase / 1000, 1));;
        ctx.beginPath();
    ...
    update(ctx) {
        var dx = mouse.p[0] - this.x;
        var dy = mouse.p[1] - this.y;
        var len = Math.sqrt(dist2(mouse.p, [
            this.x,
            this.y
        ]));
        ...
        if (distToSegment([
            this.x,
            this.y
        ], mouse.p, mouse.pp) < mouse.radius) {
            this.x -= dirX * 3;
            this.y -= dirY * 1;
        } else {
            if (Math.abs(this.x - this.baseX) > 8) this.x -= bdx / 15;
            if (Math.abs(this.y - this.baseY) > 8) this.y -= bdy / 15;
        }
        this.size = canvas.width / 200 + canvas.width / 200 / Math.max(1, distFromBase * 0.01);
    }
}
function update_particles(ctx, text, font) {
    ...
    var col = window.getComputedStyle(document.body, null).getPropertyValue("--theme-3-primary");
    gradient = col; //ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
    //gradient.addColorStop(0.3, col);
    //gradient.addColorStop(0.7, 'purple');
    const fontsize = Math.floor(Math.min(canvas.width, canvas.height) * 0.6);
    ctx.font = fontsize + "px " + font;
    ctx.fillText(text, canvas.width / 2, canvas.height / 2);
    const data = ctx.getImageData(0, 0, canvas.width, canvas.height);
    var xOffset = 0.0; //canvas.width * 0.1;
    var yOffset = 0.0;
    var newPartArr = [];
    var spacing = 10;
    var jitter = 4;
    for(let y = 0; y < data.height; y += spacing){
        for(let x = 0; x < data.width; x += spacing)if (data.data[y * 4 * data.width + x * 4] > 128) {
            var xpos = x + xOffset + Math.random() * jitter;
            var ypos = y + yOffset + Math.random() * jitter;
...
class artRenderer extends (0, _highwayDefault.default).Renderer {
    ...
    onEnterCompleted() {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        update_particles(ctx, art_strings["EN"], lang_to_font["EN"][4]);
        function animate() {
            frameCount += 1;
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            /*if (true) {  
        ...
        function mouseMove(e) {
            mouse.pp[0] = mouse.p[0];
            mouse.pp[1] = mouse.p[1];
            mouse.p[0] = e.pageX;
            mouse.p[1] = e.pageY;
        }
        function randomizeFont() {
            if (canvas != null) {
                var string_keys = Object.keys(art_strings);
                var rand = Math.floor(Math.random() * string_keys.length);
                if (rand == prevStringIndex) {
                    rand += 1;
                    rand = rand % string_keys.length;
                }
                prevStringIndex = rand;
                var key = string_keys[rand];
                var rand_font = Math.floor(Math.random() * lang_to_font[key].length);
                update_particles(ctx, art_strings[key], lang_to_font[key][rand_font]);
            }
        }
        window.onmousemove = function(e) {
            mouseMove(e);
...
},{"@dogstudio/highway":"26LRT","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["cucIK","l17dj"], "l17dj", "parcelRequire28b7")
